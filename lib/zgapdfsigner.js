...
z.PdfSigner = class{
	...
	async sign(pdf, cypopt){
		...
		/** @type {forge_cert} */
		var cert = _this.loadP12cert(_this.opt.p12cert, _this.opt.pwd, _this.opt.certificate, _this.opt.privkey, _this.opt.useP12);
		/** @type {Zga.CertsChain} */
		var cchain = null;
		if(cert){
			if(z.urlFetch){
	 ...
	 * @return {forge_cert}
	 */
	loadP12cert(p12cert, pwd, certificate, privateKey, useP12 = true) {
		const _this = this;
	
		if (useP12) {
			// Original P12 handling code
			if (!p12cert) return null;
			if (typeof p12cert !== "string") {
				p12cert = z.u8arrToRaw(new Uint8Array(p12cert));
			}
	
			const p12Asn1 = forge.asn1.fromDer(p12cert);
			const p12 = forge.pkcs12.pkcs12FromAsn1(p12Asn1, true, pwd);
			...
			if (certBags) {
				Object.keys(certBags).forEach(a_ele => {
					const a_cert = certBags[a_ele].cert;
					certs.push(a_cert);
			...
			try {
				...
				const pemCerts = pemCert.split(/(?=-----BEGIN CERTIFICATE-----)/g);
				const certs = [];
				for (const pem of pemCerts) {
					const trimmedPem = pem.trim();
					if (trimmedPem) {
						const cert = forge.pki.certificateFromPem(trimmedPem);
						certs.push(cert);
					}
				}
				...
				// Find the certificate matching the private key
				let signCert = null;
				for (const cert of certs) {
					if (privKey.n.compareTo(cert.publicKey.n) === 0 &&
						privKey.e.compareTo(cert.publicKey.e) === 0) {
						signCert = cert;
						break;
					}
				}
				...
				if (!signCert) {
					throw new Error("Certificate and private key mismatch");
				}
				...
				if (_this.cchain.getSignCert() !== signCert) {
					throw new Error("Chain of certificates is invalid.");
				}
	
				return signCert;
			} catch (err) {
				throw new Error(`PEM parsing failed: ${err.message}`);
			}
		}
	}
